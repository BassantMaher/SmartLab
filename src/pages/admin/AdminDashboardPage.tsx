...
import { Activity, Box, CheckCircle, Clock, Package, Layers, AlertTriangle } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import EnvironmentalCard from '../../components/common/EnvironmentalCard';
import RequestCard from '../../components/common/RequestCard';
import Loading from '../../components/common/Loading';
import { getEnvironmentalMetrics, getBorrowRequests, getInventoryItems, updateBorrowRequest, updateInventoryItem } from '../../utils/localStorage';
    ...
    if (requestToUpdate && user) {
      ...
      if (action === 'approve') {
        ...
        if (item) {
          const updatedItem = {
            ...item,
            availableQuantity: item.availableQuantity // Already reduced when request was created
          };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {metrics.filter(metric => metric.status !== 'normal').map((metric) => (
            <EnvironmentalCard key={metric.id} metric={metric} />
          ))}
          {metrics.filter(metric => metric.status !== 'normal').length === 0 && (
            <div className="lg:col-span-3 bg-white rounded-2xl shadow-md p-6 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-800 mb-2">All Systems Normal</h3>
              <p className="text-gray-600">
                All environmental metrics are within normal ranges.
              </p>
            </div>
          )}
        </div>
