...
import { getUserBorrowRequests } from '../../utils/localStorage';
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Get request stats
      const requests = getUserBorrowRequests(user.id);
      setTotalRequests(requests.length);
      setApprovedRequests(requests.filter(req => req.status === 'approved').length);
      setReturnedItems(requests.filter(req => req.status === 'returned').length);
          ...
          <div className="bg-white rounded-2xl shadow-md overflow-hidden">
            ...
            <div className="px-6 pb-6">
              ...
              <div className="mt-6 border-t border-gray-200 pt-4">
                <h3 className="text-sm font-medium text-gray-700 mb-3">Activity Overview</h3>
                <dl className="space-y-2">
                  <div className="flex justify-between">
                    <dt className="text-sm text-gray-500">Total Requests</dt>
                    <dd className="text-sm font-medium text-gray-900">{totalRequests}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-gray-500">Active Borrows</dt>
                    <dd className="text-sm font-medium text-gray-900">{approvedRequests}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-gray-500">Returned Items</dt>
                    <dd className="text-sm font-medium text-gray-900">{returnedItems}</dd>
                  </div>
                </dl>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 mt-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-800">Request Status Updates</h3>
                  <p className="text-xs text-gray-500">Notifications when your requests are approved or rejected</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-800">Return Reminders</h3>
                  <p className="text-xs text-gray-500">Reminders when borrowed items are due for return</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" className="sr-only peer" checked />
                  <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-[#DFF5E1] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#3B945E]"></div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-800">New Equipment Alerts</h3>
                  <p className="text-xs text-gray-500">Notifications when new equipment is added to inventory</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
