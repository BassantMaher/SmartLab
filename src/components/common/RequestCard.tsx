...
import { formatDate, getStatusColor, getStatusTextColor, getStatusBgColor } from '../../utils/helpers';
import { useAuth } from '../../context/AuthContext';

interface RequestCardProps {
  request: BorrowRequest;
  onAction?: (action: 'approve' | 'reject', requestId: string) => void;
}
    ...
    <div className={`bg-white rounded-2xl shadow-md p-5 border-l-4 ${
      ...
      {!isAdmin && request.status === 'approved' && !request.returnDate && (
        <div className="flex justify-end mt-3">
          <button className="px-3 py-1 bg-[#DFF5E1] text-[#3B945E] hover:bg-[#A8D5BA] rounded-full text-sm font-medium transition-colors duration-150">
            Mark as Returned
          </button>
        </div>
      )}
